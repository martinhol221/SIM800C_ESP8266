...
Serial.println("Conecting to: SSID:"+String(ssid)+" | PASS:"+String(pass));
...
// http://192.168.4.1/sensor?name1=TEMP4&val1=-12.24
void ds18b20_get() {
String buf = String(ESP.getChipId())+"&rssi="+String(WiFi.RSSI())+"&adc="+String(ESP.getVcc());
sensors.begin(); delay(1000); 
sensors.requestTemperatures();
for (int x=0; x < inDS; x++) buf +="&name"+String(x+1)+"=TEMP"+String(x+4)+"&val"+String(x+1)+"="+ String(sensors.getTempCByIndex(x));
get_reqest("GET /sensor?id="+buf);}
