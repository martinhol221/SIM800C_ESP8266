...
* `ussd`  - публикует в топик `car/c5/pub/ussd` баланс	`Balans=5.00r MB=4` (только GSM кодировка)
* `debug` - публикует в топик `car/c5/pub/debug` JSON строку статистики соединения `{"debug":[1,0,0,0,5,5,1]}`
* `update`- публикует в топик `car/c5/pub`	JSON строку с данными текущего состояния `{"pin":[11.57,0,0,0,0,0,0,0],"temp":[10.56,-127.00,-127.00],"time":["0:00","05:03"],"control":[1,0,1,0]}`
