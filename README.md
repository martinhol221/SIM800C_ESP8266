...
[Скачать 7.2 тест](https://github.com/martinhol221/SIM800C_ESP8266/tree/master/firmware) управление JSON строкой, новый [актуальный дашборд для MQTT Dasch](https://raw.githubusercontent.com/martinhol221/SIM800C_ESP8266/master/daschbord_7.2.txt) обязательно перезалить
...
* установит таймер обратного отсчета на 10 минут
* включит реле обходчика иммобилайзера подключенное на К5 (код 52), если авто не на передаче
* пауза 0,5 сек. (код 5)
* включит реле К2 (код 22), оно же реле ING замка зажигания, если авто не на передаче
* пауза 3 сек. (код 7)
* включит реле К1 (код 12), режим эмуляции нажатой педали тормоза или ACC, если авто не на передаче
* пауза 1 сек.(код 6)
* включит реле K3 (код 36), оно же реле STARTER замка зажигания, если измеренное напряжение ниже порогового
* выждет 1.25 сек.(код 6 + 4)
* отключит реле К3 стартера (код 30)
* пауза 3 сек. (код 7)
* отключит реле обходчика иммобилайзера подключенное на К5 (код 50)
* пауза 0,5 сек. (код 5)
* отключит реле обходчика нажатия педали тормоза на К1 (код 10)
* пауза 0,5 сек. (код 5)
* установит задержку 30 сек. перед сравнением напряжения с пороговым
* отправит параметры  в смартфон
...
* Во вкладку `ON TAP` должен быть помещен этот код 

> comand = '{"timer":24,"run":[12,6,22,7,32,6,4,30,9],"cmd":7}';
> app.publish('car/c5/sub',comand, false, 0);
