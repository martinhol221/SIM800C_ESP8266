...
Состояние входа `IN1` или `IN2` подключается на выход концевика педали тормоза (для АКПП), или на датчик включенной передачи (МКПП), и может разрешать включение реле при дополнительных кодах управления реле:
...
* При появлении импульса выше + 5в. на IN2 (датчик удара) осуществляется звонок хозяину если устройство на охране.
